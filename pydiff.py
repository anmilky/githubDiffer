...
def diff(a,b,path_data_new):
    result=[]
    lines1 = [None] + a.split('\n')
    if len (lines1) > 2000:
        pass
        # print (path_data_new + "大于2000行")
    if len(lines1)<2000:
        lines2 = [None] + b.split('\n')

    # print(lines1
    # print lines2
        dp = [[node() for i in range(len(lines2))] for j in range(len(lines1))]
    # __printDp(dp)

        dp[0][0].val = 0
    # __printDp(dp)

        for i in range(len(lines1)): dp[i][0].val=i;dp[i][0].pre_i=i-1
        for j in range(len(lines2)): dp[0][j].val=j;dp[0][j].pre_j=j-1
